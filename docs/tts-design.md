...
合成时根据 TTS 引擎的协议，将有效词典转换为对应格式：

| 方式 | 说明 | 适用引擎 |
|------|------|---------|
| SSML 注音 | `<phoneme alphabet="x-pinyin" ph="chóng qìng">重庆</phoneme>` | 支持 SSML 的商业引擎 |
| 文本替换 | 将原词替换为注音文本 | 通用方案 |
| Lexicon 文件 | 传入词典文件 | GPT-SoVITS 等本地引擎 |
...
type SynthesizeRequest struct {
    Text           string            // 纠音后的文本
    ReferenceAudio string            // 参考音频 URL
    ReferenceText  string            // 参考音频对应文本
...
| provider_type | 说明 | 典型引擎 |
|---------------|------|---------|
| local | 本地部署 | GPT-SoVITS, CosyVoice, Bark |
| online | 在线商业 | Fish Audio, 阿里 TTS, Azure TTS |
| custom | 自定义 | 用户自建服务 |
