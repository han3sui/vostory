...
声音名称有两种类型，请先判断再匹配：
A. 角色指定型：名称是人名或角色称谓（如"许一""主角""旁白"），表示该声音是专门为这个角色准备的
B. 音色描述型：名称描述音色特征（如"御姐音""少年音""沧桑大叔"），表示该声音适合具有相应特征的角色

匹配规则（按优先级排序）：
1. 名称直接匹配（最高优先级）：如果声音名称与某个角色的名称、别名相同或明显指代同一人（如声音"许一"→角色"许一"，声音"主角"→level为main的角色），则直接绑定，该声音只分配给这个角色
2. 性别必须一致：male角色只能匹配male声音，female角色只能匹配female声音，unknown性别的角色可匹配任意声音
3. 音色特征匹配：
   - 声音描述提供详细的音色特征，若存在则优先参考
   - 音色描述型的声音名称蕴含音色特征（如"少年"暗示年轻男声），作为匹配依据
   - 角色描述包含年龄、性格、身份等信息，是匹配的核心依据
   - 当角色描述为空时，根据角色名称推断其可能的特征
4. 一个音色描述型的声音可以被多个角色共用，但角色指定型的声音应专属于对应角色
