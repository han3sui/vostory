...
func NewLog(conf *viper.Viper) *Logger {
	...
	if isProd {
		// 生产环境：使用 zap.NewProduction() 的配置
		prodConfig := zap.NewProductionConfig()
		prodConfig.Level = zap.NewAtomicLevelAt(level)
		prodConfig.OutputPaths = []string{"stdout"}
		prodConfig.ErrorOutputPaths = []string{"stderr"}

		// 构建生产环境的 core，添加文件输出
		prodEncoder := zapcore.NewJSONEncoder(prodConfig.EncoderConfig)
		...
		// 使用生产环境配置创建 logger
		logger := zap.New(prodCore, zap.AddCaller(), zap.AddStacktrace(zap.ErrorLevel))
