...
import routesStore from "@/config/pinia/routes";
...
export function initGlobal() {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            global().userMenu = res.permissions;
            initRoute();
...
export function hasPermission(permission: string) {
    if (global().userInfo?.user_type === "99") {
        return true;
    }
    const permissions = global().userMenu;
...
export function hasMenu(menu: string) {
    ...
    const v = recuFind((item) => item === menu, menus);
    return v || false;
}
...
export function splitSelection(
    ...
    function traverse(node: Data): { isFullySelected: boolean; isPartiallySelected: boolean } {
        ...
        const isFullySelected = childFullySelectedCount === totalChildren && totalChildren > 0;
        const isPartiallySelected =
            !isFullySelected && (childFullySelectedCount > 0 || childPartiallySelectedCount > 0);
        ...
        // 如果子节点都没选中，父节点也不选中（即使它在 selectedIds 中）
